...
- **UI styling (user-confirmed):** Tailwind CSS v4 (`@tailwindcss/vite`) + shadcn/ui, components under `src/components/ui`. Theme tokens live in `src/styles/globals.css` (design tokens in `@theme`, shadcn `--color-*` variables in `:root`/`.dark`), structural tokens in `src/styles/tokens.css`. Visual world: DESIGN.md. Utility helper `cn()` in `src/lib/utils.ts`; dark mode via `.dark` class with `initTheme()` in `src/lib/theme.ts` (defaults to `prefers-color-scheme`).
